"""
Run:
    uv run python -m rag.stg_04_retriever "<query>"
    uv run python -m rag.stg_04_retriever "<query>" --no-rewrite   # skip query rewriting
    uv run python -m rag.stg_04_retriever "<query>" --top-k 5      # default 10
"""
...
if len(sys.argv) < 2:
    print("Usage: uv run python -m rag.stg_04_retriever \"<query>\" [--no-rewrite] [--top-k N]")
    sys.exit(1)
