"""Plain chat generation and prompt/context helpers.
...
The agentic/tool path lives in `agentic.py`, but it reuses the prompt helpers
in this file so both generation paths apply the same context filtering,
source extraction, and history update rules.
...
def build_context(results: list[SearchResult]) -> str:
    ...
    Each retrieved chunk is rendered with a stable numeric label, source title,
    optional heading path, optional URL/path, and the chunk text. The resulting
    string becomes part of the user message:

        Context:

        [C1] API Basics — Authentication
    ...
    for i, r in enumerate(results, start=1):
        # Keep metadata extraction defensive because Qdrant payloads may vary by
        # source type and older indexed chunks may not have every field.
...
def extract_sources(results: list[SearchResult]) -> list[dict]:
    """Return chunk-level source entries for UI rendering.

    Sources intentionally stay at chunk granularity so answer citations like
    `[C2]` can be mapped back to the exact retrieved section, not just a page.

    Example:
        sources = extract_sources(results)
        # → [{"id": "C1", "title": "API Basics", "path": "...", "heading": "Authentication", ...}]
    """
    out = []
    for i, r in enumerate(results, start=1):
        heading = r.metadata.get("heading", {})
...
def _citation_id(index: int) -> str:
    """Return the stable citation label used in prompt context and source UI."""
    return f"C{index}"


def validate_citations(answer: str, sources: list[dict]) -> str:
    """Mark citation IDs that do not exist in the current source list.

    This does not prove every claim is perfectly grounded, but it prevents a
    fabricated chunk citation from being displayed as if it mapped to a real
    retrieved chunk.
    """
...
def prepare_prompt(query: str, results: list[SearchResult], cfg: ChatConfig) -> tuple[list[SearchResult], str, str]:
    ...
    """
    filtered = [r for r in results if (r.rerank_score or 0) >= cfg.min_rerank_score]
