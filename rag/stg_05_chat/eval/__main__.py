...
Examples:
    ...
    uv run python -m rag.stg_05_chat.eval --golden files/eval/rag_golden.jsonl
    uv run python -m rag.stg_05_chat.eval --retrieval-only
    uv run python -m rag.stg_05_chat.eval --judge
    uv run python -m rag.stg_05_chat.eval \
...
from rag.stg_04_retriever.query_rewriter import rewrite
from rag.stg_04_retriever.retriever import rerank, retrieve
from rag.stg_05_chat.config import ChatConfig
from rag.stg_05_chat.generator import generate
...
def main() -> None:
    ...
    store = QdrantStore(url=settings.qdrant_url, collection=args.collection or settings.collection)
    ...
    print(f"\n{'═' * W}")
    print("  RAG EVAL")
    print(f"  cases={len(cases)}  collection={store.collection}  model={args.provider}/{args.model}  judge={'on' if args.judge else 'off'}")
    print(f"{'═' * W}")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--collection", default="", help="Qdrant collection name. Defaults to config.settings.Settings.collection")
