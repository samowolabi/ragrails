"""Ragrails CLI — run ingestion commands from the terminal."""
...
from ragrails.sdk import RagRails


@click.group()
@click.version_option(package_name="ragrails")
def cli():
    """Ragrails — ingest, chunk, and store content for RAG pipelines."""
...
def scrape(url, mode, output_dir, max_depth, max_pages, no_frontmatter):
    ...
    Examples:
      ...
      ragrails scrape https://example.com --mode full --max-pages 50
...
def fetch(url, title, description, method, headers, params, output_dir, max_pages, no_frontmatter):
    ...
    Examples:
      ...
      ragrails fetch https://api.example.com/v1/products --header "Authorization:Bearer token" --header "X-Api-Key:my-key"
    ...
    try:
        parsed_headers = _parse_pairs(headers)
        parsed_params = _parse_pairs(params)
