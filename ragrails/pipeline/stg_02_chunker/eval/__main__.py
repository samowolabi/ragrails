"""
Run:
    uv run python -m ragrails.pipeline.stg_02_chunker.eval                        # structural eval — all chunk files
    uv run python -m ragrails.pipeline.stg_02_chunker.eval file <name>            # structural eval — single file + flagged detail
    uv run python -m ragrails.pipeline.stg_02_chunker.eval llm openai             # LLM eval — gpt-4o-mini
    uv run python -m ragrails.pipeline.stg_02_chunker.eval llm claude             # LLM eval — claude-haiku-4-5-20251001
    uv run python -m ragrails.pipeline.stg_02_chunker.eval llm openai <n>         # LLM eval — sample n chunks (default 10)
    uv run python -m ragrails.pipeline.stg_02_chunker.eval llm claude <n>
"""
...
if mode == "all":
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run python -m ragrails.pipeline.stg_02_chunker.eval file <path>")
        sys.exit(1)
    ...
    if len(sys.argv) < 3 or sys.argv[2] not in ("openai", "claude"):
        print("Usage: uv run python -m ragrails.pipeline.stg_02_chunker.eval llm openai|claude [n]")
        sys.exit(1)
